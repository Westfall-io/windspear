...
def parse_models(models, elements):
            ...
            for a in subactions:
                ...
                if actions[a.get_id()]['tools']['harbor'] is not None and \
                    ...
                    for i in inputs.get_elements():
                        ...
                        if not 'variables' in actions[a.get_id()]['tools']['artifacts']:
                            actions[a.get_id()]['tools']['artifacts']['variables'] = {}

                        for tv in tool_variables:
                            actions[a.get_id()]['tools']['artifacts']['variables'][tv] = {
                                'value': val_variable,
                                'units': units
                            }
            ...
            if '::' in e.get_key('qualifiedName'):
                if not 'obj' in e.get_key('qualifiedName').split('::'):
                    requirements[e.get_id()] = {
                        'name': e.get_key('qualifiedName'),
                    }

    print('\n\n\n\n---------------------')
    print('Listing all verifications found.')
    for element_id in verifications:
        print('Verification: {}'.format(verifications[element_id]['name']))
        for reqt in verifications[element_id]['requirements']:
            print('   validates Requirement {}'.format(requirements[reqt['@id']]['name']))
    ...
    for element_id in requirements:
        print('Requirement: {}'.format(requirements[element_id]['name']))
