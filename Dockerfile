...
RUN apt-get update && apt-get install -y libpq-dev gcc wget unzip git default-jre default-jdk graphviz
RUN pip install "psycopg[binary]"
COPY ./requirements.txt ./requirements.txt
RUN pip install --upgrade pip && pip install -r requirements.txt

## Download and install SysML kernel
RUN wget https://github.com/Systems-Modeling/SysML-v2-Pilot-Implementation/releases/download/2024-11/jupyter-sysml-kernel-0.46.0.zip
RUN unzip jupyter-sysml-kernel-0.46.0.zip -d /tmp \
